...
/// referenced from http://www.euclideanspace.com/maths/geometry/rotations/conversions/matrixToAngle/
template <class T>
inline void RotationMatrix2AngleAxis(T* rotation, T& angle_in_radian, T* axis) {
  T (*m)[3] = (T (*)[3]) rotation;
  T x, y, z; // variables for result
  T epsilon = 0.01; // margin to allow for rounding errors
  T epsilon2 = 0.1; // margin to distinguish between 0 and 180 degrees
  // optional check that input is pure rotation, 'isRotationMatrix' is defined at:
  // http://www.euclideanspace.com/maths/algebra/matrix/orthogonal/rotation/
  if ((dj::Abs(m[0][1] - m[1][0]) < epsilon)
      && (dj::Abs(m[0][2] - m[2][0]) < epsilon)
      && (dj::Abs(m[1][2] - m[2][1]) < epsilon)) {
    // singularity found
    // first check for identity matrix which must have +1 for all terms
    //  in leading diagonaland zero in other terms
    if ((dj::Abs(m[0][1] + m[1][0]) < epsilon2)
        && (dj::Abs(m[0][2] + m[2][0]) < epsilon2)
        && (dj::Abs(m[1][2] + m[2][1]) < epsilon2)
        && (dj::Abs(m[0][0] + m[1][1] + m[2][2] - 3) < epsilon2)) {
  ...
  T s = sqrt((m[2][1] - m[1][2]) * (m[2][1] - m[1][2])
                       + (m[0][2] - m[2][0]) * (m[0][2] - m[2][0])
                       + (m[1][0] - m[0][1]) * (m[1][0] - m[0][1])); // used to normalise
  if (dj::Abs(s) < 0.001) s = 1;
  // prevent divide by zero, should not happen if matrix is orthogonal and should be
  // caught by singularity test above, but I've left it in just in case
  angle_in_radian = acos((m[0][0] + m[1][1] + m[2][2] - 1) / 2);
