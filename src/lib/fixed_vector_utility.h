...
inline T Normalize2(T* vector)
{
  ...
  T invNorm = 1 / norm ;
  if (invNorm != invNorm) {
    std::cerr << CURRENT_LINE << " => try to normalize a 0 vector." << std::endl;
    exit(0);
  }
  vector[0] *= invNorm ;
  vector[1] *= invNorm ;
...
inline T Normalize3(T* a)
{
  ...
  T invNorm = 1 / norm ;
  ASSERT(invNorm == invNorm, P(a[0], a[1], a[2]));
//  if (invNorm != invNorm) {
...
{
  const float kEpsilon = 1e-18f;
  const T det = Determinant4(p0, p1, p2, p3);
  //  if (det < EPSILON && det > -EPSILON) {
  if (det < kEpsilon && det > -kEpsilon) {
    std::cerr << CURRENT_LINE << " => Tet volume (" << det << ") is almost flat!!" << std::endl;
    return false;
  }
  const T det_inv = T(1.0) / det;
  barycentricCoord[0] = Determinant4(p, p1, p2, p3) * det_inv;
...
struct FixedVecOp {
  ...
  inline static T Normalize(T* x)
  {
    ...
    if (length_inv != length_inv) {
      std::cerr << CURRENT_LINE << " => try to normalize 0 vector." << std::endl;
      exit(-1);
      return T(0);
    }
    for (int i = 0; i < d; ++i) {
