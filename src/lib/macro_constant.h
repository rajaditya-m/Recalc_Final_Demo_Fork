...
inline std::string GetFileNameFromFullPath(const char* full_path) {
  int i = 0;
  int last_slash = -1;
#ifdef _MSC_VER
...
#endif
  for (; full_path[i] != '\0'; ++i) {
    if (full_path[i] == kDirSeparator) last_slash = i;
  }
  if (last_slash == -1) {
    return std::string(full_path);
  } else {
    return std::string(full_path + last_slash + 1);
  }
}

#define CURRENT_LINE (GetFileNameFromFullPath(__FILE__) + std::string(":") + ToString(__LINE__) + std::string(":") + std::string(FUNCTION_NAME))
