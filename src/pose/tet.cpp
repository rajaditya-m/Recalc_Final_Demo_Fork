...
Tet::Tet(const char *filename, double _limit_threshold, AffineTransformer<double>* affine_transformer, bool initialize_fem_module, bool load_interface, int split)
  : vertex_num_(0) {
  P(filename);
  higher_sampler_ = NULL;
  lower_sampler_ = NULL;
  domain_offset_toggle_ = 6654;
  skeleton_ = NULL;
  ...
  render_X = new double[vertex_num_ * 3];
  memcpy(X, &vert_pos[0], sizeof(double) * vertex_num_ * 3);
  memcpy(render_X, &vert_pos[0], sizeof(double) * vertex_num_ * 3);
  ...
  if (initialize_fem_module) {
    inv_fem_ = new TetMeshSimulatorBridge(this,interface_vertices_, split);
  } else {
    inv_fem_ = NULL;
  }
...
void Tet::Render(int render_mode, double * pos) {
  ...
  if ((render_mode & kSmoothLighting) || (render_mode & kFlatLighting)) {
    if (textured_surface_renderer_.size() != 0) {
      ...
      //float diffuse_color[4] = { 0.98, 0.96, 0.93,1.0f};
      //float diffuse_color1[4] = { 0.0, 0.96, 0.93,1.0f};
      //float diffuse_color2[4] = { 0.98, 0.0, 0.93,1.0f};
      ...
      /*if(global::sim_state==3) {
          for(auto it=interface_vertices_.begin(); it!= interface_)
      }*/
      ...
      for (int i = 0; i < triangle_num_; i++) {
        if(global::sim_state!=3) {
            ...
            if(i<last_tri) {
                glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, color_map[1]);
            }
            else {
                glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, color_map[3]);
            }
            ...
            double *p0 = &pos[T_Stitch_[i * 3 + 0] * 3];
            double *p1 = &pos[T_Stitch_[i * 3 + 1] * 3];
            double *p2 = &pos[T_Stitch_[i * 3 + 2] * 3];
           // glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, diffuse_color);
            ...
            glNormal3f(VN[T_Stitch_[i * 3 + 0] * 3 + 0], VN[T_Stitch_[i * 3 + 0] * 3 + 1], VN[T_Stitch_[i * 3 + 0] * 3 + 2]);
            Vertex3v(p0);
            glNormal3f(VN[T_Stitch_[i * 3 + 1] * 3 + 0], VN[T_Stitch_[i * 3 + 1] * 3 + 1], VN[T_Stitch_[i * 3 + 1] * 3 + 2]);
            Vertex3v(p1);
            glNormal3f(VN[T_Stitch_[i * 3 + 2] * 3 + 0], VN[T_Stitch_[i * 3 + 2] * 3 + 1], VN[T_Stitch_[i * 3 + 2] * 3 + 2]);
            Vertex3v(p2);
        }
...
void Tet::Build_Boundary_Triangles() {
  ...
  incident_triangle_renderer_ = incident_triangle_;

  std::string file_name = dj::Format("%z/inter.bou",GetDataFolder());
  P(file_name);
  int numIntV;
  int* IntV;
  loadCommaList(file_name.c_str(),&numIntV,&IntV);
  std::unordered_set<int> tableVertices;
  for(int i = 0 ;i < numIntV; i++) {
  ...
  incident_triangle_renderer__2 = incident_triangle_renderer_;

  for(auto it= interface_vertices_.begin(); it!= interface_vertices_.end();it++) {
      int j = it->second;
      int i = it->first;

      std::vector<int> incidence_vector_j = incident_triangle_renderer__2[j];
      std::vector<int> incidence_vector_i = incident_triangle_renderer__2[i];

      for(auto it = incidence_vector_j.begin();it != incidence_vector_j.end();it++ ) {
          int t = *it;
          if(T_Stitch_[t*3+0] == j) {
              T_Stitch_[t*3+0] = i;
          } else if (T_Stitch_[t*3+1] == j) {
              T_Stitch_[t*3+1] = i;
          } else {
              T_Stitch_[t*3+2] = i;
          }
      ...
      //std::cout << "Before : " << incident_triangle_renderer__2[i].size() << " " << incident_triangle_renderer__2[j].size() << "\n";

      //Merge all of them to a new vector
      std::vector<int> newV = incidence_vector_i;
      newV.insert(newV.end(),incidence_vector_j.begin(),incidence_vector_j.end());
      std::sort( newV.begin(), newV.end() );
      newV.erase( std::unique( newV.begin(), newV.end() ), newV.end() );

      incident_triangle_renderer__2[i] = newV;
      incident_triangle_renderer__2[j].clear();
  ...
  for(int t = 0;t<triangle_num_;t++) {
      ...
      int b = T[t*3+1];
      int c = T[t*3+2];
      if((a >= domain_offset_toggle_ || b >= domain_offset_toggle_ || c >= domain_offset_toggle_) && (last_tri== -1)) {
          last_tri = t;
      }
  }
  P(last_tri);
  ...
  for (int i = 0; i < int(surface_vertices_.size()); ++i) {
    int global_v = surface_vertices_[i];
    if(global_v < domain_offset_toggle_) {
        surface_vert_colors_[i * 3 + 0] = color[colIdx1 * 3 + 0];
        surface_vert_colors_[i * 3 + 1] = color[colIdx1 * 3 + 1];
        surface_vert_colors_[i * 3 + 2] = color[colIdx1 * 3 + 2];
    }
    else {
        surface_vert_colors_[i * 3 + 0] = color[colIdx2 * 3 + 0];
        surface_vert_colors_[i * 3 + 1] = color[colIdx2 * 3 + 1];
        surface_vert_colors_[i * 3 + 2] = color[colIdx2 * 3 + 2];
    }
  }
...
void Tet::Build_TN() {
  ...
  OMP_FOR
  for (int i = 0; i < triangle_num_; i++) {
    double*p0 = &render_X[T[i * 3 + 0] * 3];
    double*p1 = &render_X[T[i * 3 + 1] * 3];
    double*p2 = &render_X[T[i * 3 + 2] * 3];
...
void Tet::Build_VN() {
  ...
  /*for (int v = 0; v < vertex_num_; ++v) {
    ...
    if(global::sim_state != 3) {
        ...
        for (auto tri : incident_triangle_renderer__2[v]) {
            normal[0] += TN[tri * 3 + 0];
            normal[1] += TN[tri * 3 + 1];
            normal[2] += TN[tri * 3 + 2];
        }
