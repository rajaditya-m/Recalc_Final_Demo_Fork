...
public:
  ...
  public:
    explicit Material(const std::string name_,
                      const Vec3d & Ka_, const Vec3d & Kd_, const Vec3d & Ks_,
                      double shininess_ = 0,
                      const std::string textureFilename_ = std::string(),
                      const std::string bumpMapFileName_ = std::string()):
      Ka(Ka_), Kd(Kd_), Ks(Ks_),
      shininess(shininess_),
      alpha(1.0),
      name(name_),
      textureFilename(textureFilename_),
      bumpMapFileName(bumpMapFileName_)
    {}

    explicit Material():
      Ka(Vec3d(1, 1, 1)), Kd(Vec3d(1, 1, 1)), Ks(Vec3d(1, 1, 1)), shininess(0),
      alpha(1.0),
      name(std::string("default")),
      textureFilename(std::string()),
      bumpMapFileName(std::string())
    {}
    ...
    inline void setBumpMapFilename(const std::string& bumpMapFileName_) { bumpMapFileName = bumpMapFileName_; }
    inline void setBumpMapFilename(const char *bumpMapFileName_) { bumpMapFileName = std::string(bumpMapFileName_); }
    ...
    inline bool hasBumpMapFilename() const { return bumpMapFileName.size() > 0; }
    ...
    inline std::string getBumpMapFilename() const { return bumpMapFileName; }
  ...
  inline void addMaterial(const std::string name, const Vec3d & Ka, const Vec3d & Kd, const Vec3d & Ks,
                          ...
                          const std::string bumpTextureFilename = std::string()) {
    materials.push_back(Material(name, Ka, Kd, Ks, shininess, textureFilename, bumpTextureFilename));
  }
