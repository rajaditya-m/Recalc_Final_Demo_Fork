...
public:
  ...
  public:
    Texture()
      : fullPath(std::string("__none"))
      , texture(std::make_pair(false, 0))
      , textureMode(OBJMESHRENDER_GL_NOMIPMAP | OBJMESHRENDER_GL_MODULATE)
      , bytesPerPixel(3) {}
    virtual ~Texture();

    static void loadTextureImage(std::string fullPath, int * width, int * height, int * bpp, unsigned char ** texData);

    void loadTexture(std::string fullPath, int textureMode, bool convert2NormalMap = false); // also sets up OpenGl
  ...
  Texture * getBumpTextureHandle(int textureIndex);
