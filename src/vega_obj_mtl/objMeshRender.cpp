...
void ObjMeshRender::Texture::loadTextureImage(string fullPath, int * width, int * height, int * bpp, unsigned char ** texData) {
  QImageReader reader;
  QString qName(fullPath.c_str());
  reader.setFileName(qName);
  ASSERT(reader.canRead(), L(fullPath));
  QImage img = reader.read().convertToFormat(QImage::Format_RGB888).mirrored(false, true);
  *width = img.width();
  *height = img.height();
  *bpp = 3;
  *texData = (unsigned char*) malloc (sizeof(unsigned char) **width **height **bpp);
  memcpy(*texData, img.bits(), sizeof(unsigned char) **width **height **bpp);
}

//#include <QImage>
//#include <QImageWriter>
void ObjMeshRender::Texture::loadTexture(string fullPath, int textureMode_, bool convert2NormalMap) {
  ...
  if (convert2NormalMap) {
    Texture2NormalMap(texData, width, height, 2.0);
    {
#if 0
      static int i = 0;
      ++i;
      ...
      memcpy(image->bits(), texData,  width * height * bytesPerPixel);
      QString name = QString("/Users/dj/bump_%1.bmp").arg(i);
//      qDebug() << name;
      image_writer = new QImageWriter(name, "bmp");
...
void ObjMeshRender::loadTextures(int textureMode, std::vector<Texture*> * texturePool, int updatePool) {
  ...
  textures.clear();
  for (int i = 0; i < (int)bumpTextures.size(); i++) {
    if (ownBumpTexture[i]) {
      delete(bumpTextures[i]);
      ownBumpTexture[i] = 0;
    }
  }
  bumpTextures.clear();
  ...
  bumpTextures.resize(numMaterials);
  ownBumpTexture.resize(numMaterials);
  ...
  for (int i = 0; i < numMaterials; i++) {
    ...
    string bump_texture_name = pathStr + string("/") + material->getBumpMapFilename();

    int found = 0;
    int found_bump = 0;
    texturePool = NULL;
    ASSERT(glGetError() == GL_NO_ERROR, P(i));
    if (texturePool != NULL) {
      ...
      for (int j = 0; j < (int)texturePool->size(); j++) {
        ...
        if (!found && fullPath == poolFullPath) {
        ...
        if (!found_bump && bump_texture_name == poolFullPath) {
          if (material->hasBumpMapFilename()) {
            bumpTextures[i] = (*texturePool)[j];
            ownBumpTexture[i] = 0;
            found_bump = 1;
          }
        }
    ...
    if (!found_bump && material->hasTextureFilename()) {
      bumpTextures[i] = new Texture();
      ownBumpTexture[i] = 1;
      bumpTextures[i]->loadTexture(fullPath, textureMode, true);
      if (updatePool)
        texturePool->push_back(bumpTextures[i]);
    }
  }
}
...
ObjMeshRender::Texture *ObjMeshRender::getBumpTextureHandle(int textureIndex) {
  return bumpTextures[textureIndex];
}
