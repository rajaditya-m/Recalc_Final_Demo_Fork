    ...
    switch (buf[0]) {
      ...
      case 'n':
        ...
        if (numMaterials >= 1) // flush previous material
          addMaterial(matName, Vec3d(Ka[0], Ka[1], Ka[2]), Vec3d(Kd[0], Kd[1], Kd[2]), Vec3d(Ks[0], Ks[1], Ks[2]), shininess, textureFile, bumpMapFile);
      ...
      case 'm':
        ...
        if (strcmp(buf, "map_bump") == 0) {
          fgets_(buf, sizeof(buf), file);
          sscanf(buf, "%s %s", buf, buf);
          bumpMapFile = string(buf);
          if (verbose)
            printf("Noticed bump map \"%s\".\n", bumpMapFile.c_str());
        }
        break;
