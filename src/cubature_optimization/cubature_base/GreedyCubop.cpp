...
void GreedyCubop::gatherRandomUnusedPoints( vector<int>& usedPoints, vector<int>& candidatesOut, int maxCands ) {
  ...
  for ( int i = 0; i < int(usedPoints.size()); i++ )
    usedPointsSet.insert( usedPoints[i] );

  while (int(candidatesOut.size()) < maxCands && int(usedPointsSet.size()) < numTotalPoints()) {
...
void GreedyCubop::onFinishOptimizingCubature(std::vector<int> &selectedPoints, VECTOR &weights, Real relErr) {
  (void) selectedPoints;
  (void) weights;
  (void) relErr;
}

void write_col_major_matrix( double* A, int m, int n, ostream& os ) {
  os << "[";
  for ( int i = 0; i < m; i++ ) {
    for ( int j = 0; j < n; j++ ) {
      os << A[ j * m + i ] << " ";
...
void GreedyCubop::evalPointColumn(
  ...
  for ( int t = 0; t < int(trainingSet.size()); t++ ) {
    //		evalPointForceDensity( pointId, *trainingSet[t], g );
    evalPointForceDensity( pointId, *trainingSet[t], g, t);
...
int GreedyCubop::findMostFitPoint(
  ...
  const int T = int(trainingSet.size());
  const int nCands = int(candidatePoints.size());

  assert(residual.size() == _r * T);
  assert(int(inverseForceNorms.size()) == T);
...
void GreedyCubop::selectCandidatePoints(
  ...
  if ( numCandidates >= int(numTotalPoints() - usedPoints.size()) ) {
    // just grab em all
...
void GreedyCubop::run(
  ...
  //  Start training
  //----------------------------------------
  // TRACE(0,"Allocating matrix of %d x %d x %d bytes per Real = %d = %f MB. If this is too large, it could core-dump here.", _r*T, maxNumPoints, sizeof(Real), _r*T*maxNumPoints*sizeof(Real), _r*T*maxNumPoints*sizeof(Real) / 1024.0 / 1024.0 );
  MATRIX A( _r * T, maxNumPoints );
  A.clear();
  // TRACE(0,"Matrix allocation successful! No core-dump - OK.");
...
//  cout << "bNorm = " << bNorm << " should be sqrt(T) = " << sqrt(T) << endl;
  ...
  // TRACE(0,"Entering cubop loop");

  while ( relErr > relErrTol && int(selectedPoints.size()) < maxNumPoints && int(selectedPoints.size()) < numTotalPoints()) {
    ...
    fill_range( subsetIndices, 0, int(trainingSet.size()) );
    ...
    for ( int i = 0; i < itersPerFullNNLS && int(selectedPoints.size()) < maxNumPoints; i++ ) {
      //----------------------------------------
      ...
      A.setVector( bestPointCol, 0, int(selectedPoints.size()) - 1 );
      ...
      subsetA.clear();
      for ( int i = 0; i < int(subsetIndices.size()); i++ ) {
      ...
      w.resizeAndWipe( int(selectedPoints.size()) );

      // go!
      bool converged = subNNLS.solveRowMajor(
                         subsetA.data(),
                         subsetA.cols(),
                         int(selectedPoints.size()),
                         subsetB,
                         w,
                         residualNorm );
      (void) converged;
  ...
  onFinishOptimizingCubature(selectedPoints, w, relErr);
}
...
void GreedyCubop::handleCubature( std::vector<int>& selectedPoints, VECTOR& weights, Real relErr ) {
  ...
  (void) relErr;
  std::ofstream fileOut("cubature_weights.txt");
...
void GreedyCubop::runNQP(
  ...
  while ( relErr > relErrTol && int(selectedPoints.size()) < maxNumPoints && int(selectedPoints.size()) < numTotalPoints() ) {
    ...
    const int n = int(selectedPoints.size());
