...
void OpenGLSoftShadowRender::RenderSoftShadow() {
  ...
  GLuint ap1loc = glGetUniformLocation(shadow_program_, "ap1");
  glUniform1f(ap1loc,global::ap1);
  GLuint ap2loc = glGetUniformLocation(shadow_program_, "ap2");
  glUniform1f(ap2loc,global::ap2);
  glUniform3fv(uniloc, 1, light_position_);
  //    glUseProgram(0);
  const float kHeight = -0.8f;
  const float kTextureScale = 5.0f;
