...
TetMeshSimulatorBridge::TetMeshSimulatorBridge(Tet *tet_mesh,std::vector<std::pair<int,int> > &interfaceV, int split) {
   sim_mode_ = 1;
  ...
  element_k_ = NULL;
  if(split) {
      int totalSize = interfaceV.size();
      interface_points_1_.resize(split);
      interface_points_2_.resize(totalSize);
      std::copy(interfaceV.begin(),interfaceV.begin()+split,interface_points_1_.begin());
      std::copy(interfaceV.begin(),interfaceV.end(),interface_points_2_.begin());
  }
  else {
    interface_points_1_.resize(interfaceV.size());
    std::copy(interfaceV.begin(),interfaceV.end(),interface_points_1_.begin());
  }
  ...
  inv_fem_force_model_->GetStiffnessMatrixTopology(&stitch_tangent_stiffness_matrix_1_,interface_points_1_);
  inv_fem_force_model_->GetStiffnessMatrixTopology(&stitch_tangent_stiffness_matrix_2_,interface_points_2_);

  //Precompute this
  double *u_rest = new double[3 * tet_mesh_->vertex_num_];
  memset(u_rest, 0, sizeof(double) * 3 * tet_mesh_->vertex_num_);
  inv_fem_force_model_->GetTangentStiffnessMatrix((real*) u_rest, stitch_tangent_stiffness_matrix_1_);
   inv_fem_force_model_->GetTangentStiffnessMatrix((real*) u_rest, stitch_tangent_stiffness_matrix_2_);
  element_force_ = inv_fem_force_model_->elementForce;
  element_k_ = inv_fem_force_model_->elementK;

  //Compute the spring matrix
  SparseMatrixOutline *sp_mat_out = new SparseMatrixOutline((3 * v_num_));
  for(auto it=interface_points_1_.begin();it != interface_points_1_.end();it++) {
      int i = it->first;
      int j = it->second;
      sp_mat_out->AddEntry( 3 * i + 0, 3 * i + 0, 5.0e5 );
      sp_mat_out->AddEntry( 3 * i + 1, 3 * i + 1, 5.0e5 );
      sp_mat_out->AddEntry( 3 * i + 2, 3 * i + 2, 5.0e5 );

      sp_mat_out->AddEntry( 3 * j + 0, 3 * j + 0, 5.0e5 );
      sp_mat_out->AddEntry( 3 * j + 1, 3 * j + 1, 5.0e5 );
      sp_mat_out->AddEntry( 3 * j + 2, 3 * j + 2, 5.0e5 );

      sp_mat_out->AddEntry( 3 * i + 0, 3 * j + 0, -5.0e5 );
      sp_mat_out->AddEntry( 3 * i + 1, 3 * j + 1, -5.0e5 );
      sp_mat_out->AddEntry( 3 * i + 2, 3 * j + 2, -5.0e5 );

      sp_mat_out->AddEntry( 3 * j + 0, 3 * i + 0, -5.0e5 );
      sp_mat_out->AddEntry( 3 * j + 1, 3 * i + 1, -5.0e5 );
      sp_mat_out->AddEntry( 3 * j + 2, 3 * i + 2, -5.0e5 );
  }

  spring_mat_ = new SparseMatrix(sp_mat_out);
  delete sp_mat_out;
...
void TetMeshSimulatorBridge::makeSpringConnections() {
    if(sim_mode_==2) {
        ...
        for(auto it = interface_points_2_.begin(); it!= interface_points_2_.end(); it++) {
            this->makeLinks(it->first, it->second);
        }
        //P("This shoiuld not be there..");
    }
}

void TetMeshSimulatorBridge::makeLinks(int i, int j) {

    double k = 5.0e7;// * global::stiffyMult;

    int colIdx;
    SparseMatrix* stitch_tangent_stiffness_matrix_;
    if(sim_mode_==2) {
        stitch_tangent_stiffness_matrix_ = stitch_tangent_stiffness_matrix_1_;
    }
    else if(sim_mode_==4) {
        stitch_tangent_stiffness_matrix_ = stitch_tangent_stiffness_matrix_2_;
    }
...
TetMeshSimulatorBridge::~TetMeshSimulatorBridge() {
  ...
  delete stitch_tangent_stiffness_matrix_1_;
  delete stitch_tangent_stiffness_matrix_2_;
  delete cg_solver_;
  delete material_;
...
void TetMeshSimulatorBridge::Reset() {
  ...
  inv_fem_force_model_->GetTangentStiffnessMatrix((real*) u_, stitch_tangent_stiffness_matrix_1_);
}

SparseMatrix* TetMeshSimulatorBridge::GetStitchStiffnessMatrixPointer(int ID) {
    if(global::stiffyMult!=1) {
        for(int i = 0; i<6142*3;i++) {
            stitch_tangent_stiffness_matrix_1_->MultiplyRow(i,global::stiffyMult);
        }
    }
    makeSpringConnections();
    return stitch_tangent_stiffness_matrix_1_;
}
...
void TetMeshSimulatorBridge::ComputeInternalForceAndTangentStiffnessMatrix(double dt) {
  ...
  if(sim_mode_==1)
    inv_fem_force_model_->GetForceAndTangentStiffnessMatrix((real*) u_, (real*) internal_force_, tangent_stiffness_matrix_);
      ...
      inv_fem_force_model_->GetForceAndTangentStiffnessMatrix((real*) u_, (real*) internal_force_, stitch_tangent_stiffness_matrix_1_);
      makeSpringConnections();
  }
  else if(sim_mode_==4) {
      inv_fem_force_model_->GetForceAndTangentStiffnessMatrix((real*) u_, (real*) internal_force_, stitch_tangent_stiffness_matrix_2_);
      makeSpringConnections();
  }
#endif
