...
void SparseMatrixOutline::AddBlockMatrix(int i, int j, int blockX, int blockY) {
//    if (i == 1373) {
//      P(j);
//    }
...
void SparseMatrix::MultiplyVector(const double * vector, double * result) const {
  for (int i = 0; i < numRows; i++) {
    result[i] = 0;
    for (int j = 0; j < rowLength[i]; j++) {
//      if (columnIndices[i][j] < 0) {
//        P(i, j, columnIndices[i][j], rowLength[i]);
//      }
//      if (i == 0) {
//        P(j, columnIndices[i][j]);
//      }
      result[i] += vector[columnIndices[i][j]] * columnEntries[i][j];
      //      int col = columnIndices[i][j];
      //      if (vector[col] != 0 && i == 20892 * 3) {
      //        P(columnEntries[i][j], vector[col]);
      //      }
    }
  }
}
