  ...
  *elementType_ = INVALID;
  ...
  while (volumetricMeshParser.getNextLine(lineBuffer, 0, 0) != NULL)
  {
    ...
    if ((parseState == 0) && (strncmp(lineBuffer, "*MATERIAL", 9) == 0))
    {
      ...
      unsigned int materialTypeLength = uint(ch - materialSpecification);
...
{
  return (unsigned int) fread(buf, elementSize, numElements, (FILE*)fin);
}
...
int VolumetricMesh::saveToBinary(FILE * binaryOutputStream, unsigned int * bytesWritten, elementType elementType_, bool countBytesOnly) const
{
  ...
  itemsWritten = 1;
  if (!countBytesOnly)
    itemsWritten = (unsigned int) fwrite(&version, sizeof(double), 1, binaryOutputStream);
  if (itemsWritten < 1)
    return 1;
  ...
  itemsWritten = 1;
  if (!countBytesOnly)
    itemsWritten = (unsigned int) fwrite(&eleType, sizeof(int), 1, binaryOutputStream);
  if (itemsWritten < 1)
    return 1;
  ...
  itemsWritten = 1;
  if (!countBytesOnly)
    itemsWritten = (unsigned int) fwrite(&numVertices, sizeof(int), 1, binaryOutputStream);
  if (itemsWritten < 1)
    return 1;
  totalBytesWritten += itemsWritten * sizeof(int);
  ...
  for(int vertexIndex = 0; vertexIndex < numVertices; vertexIndex++)
  {
    ...
    itemsWritten = 3;
    if (!countBytesOnly)
      itemsWritten = (unsigned int)fwrite(v_array, sizeof(double), 3, binaryOutputStream);
    if (itemsWritten < 3)
      return 1;
    totalBytesWritten += itemsWritten * sizeof(double);
  }
  ...
  itemsWritten = 1;
  if (!countBytesOnly)
    itemsWritten = (unsigned int)fwrite(&numElements, sizeof(int), 1, binaryOutputStream);
  if (itemsWritten < 1)
    return 1;
  totalBytesWritten += itemsWritten * sizeof(int);
  ...
  itemsWritten = 1;
  if (!countBytesOnly)
    itemsWritten = (unsigned int)fwrite(&numElementVertices, sizeof(int), 1, binaryOutputStream);
  if (itemsWritten < 1)
    return 1;
  totalBytesWritten += itemsWritten * sizeof(int);
  ...
  for(int elementIndex = 0; elementIndex < numElements; elementIndex++)
  {
    for(int vertexIndex = 0; vertexIndex < numElementVertices; vertexIndex++)
    {
      ...
      itemsWritten = 1;
      if (!countBytesOnly)
        itemsWritten = (unsigned int)fwrite(&temp, sizeof(int), 1, binaryOutputStream);
      if (itemsWritten < 1)
        return 1;
      totalBytesWritten += itemsWritten * sizeof(int);
    } 
  ...
  itemsWritten = 1;
  if (!countBytesOnly)
    itemsWritten = (unsigned int)fwrite(&numMaterials, sizeof(int), 1, binaryOutputStream);
  if (itemsWritten < 1)
    return 1;
  totalBytesWritten += itemsWritten * sizeof(int);
  ...
  for(int materialIndex = 0; materialIndex < numMaterials; materialIndex++)
  {
    ...
    itemsWritten = 1;
    if (!countBytesOnly)
      itemsWritten = (unsigned int)fwrite(&length, sizeof(int), 1, binaryOutputStream); // write the length of the material name
    if (itemsWritten < 1)
      return 1;
    totalBytesWritten += itemsWritten * sizeof(int);
    ...
    itemsWritten = length;
    if (!countBytesOnly)
      itemsWritten = (unsigned int)fwrite(name, sizeof(char), length, binaryOutputStream);
    ...
    itemsWritten = 1;
    if (!countBytesOnly)
      itemsWritten = (unsigned int)fwrite(&matType, sizeof(int), 1, binaryOutputStream);
    if (itemsWritten < 1)
      return 1;
    totalBytesWritten += itemsWritten * sizeof(int);
    switch (matType)
    {
      case Material::ENU:
      {
        ...
        if (!countBytesOnly)
          itemsWritten = (unsigned int)fwrite(materialProperty, sizeof(double), Material::ENU_NUM_PROPERTIES, binaryOutputStream);
      ...
      case Material::MOONEYRIVLIN:
      {
        ...
        if (!countBytesOnly)
          itemsWritten = (unsigned int)fwrite(materialProperty, sizeof(double), Material::MOONEYRIVLIN_NUM_PROPERTIES, binaryOutputStream);
      ...
      case Material::ORTHOTROPIC:
      {
        ...
        if (!countBytesOnly)
          itemsWritten = (unsigned int) fwrite(materialProperty, sizeof(double), Material::ORTHOTROPIC_NUM_PROPERTIES, binaryOutputStream);
        ...
        itemsWritten = 9;
        if (!countBytesOnly)
          itemsWritten = (unsigned int)fwrite(R, sizeof(double), 9, binaryOutputStream);
        if (itemsWritten < 9)
          return 1;
        totalBytesWritten += itemsWritten * sizeof(double);
      }
  ...
  itemsWritten = 1;
  if (!countBytesOnly)
    itemsWritten = (unsigned int)fwrite(&numSets, sizeof(int), 1, binaryOutputStream);
  if (itemsWritten < 1)
    return 1;
  totalBytesWritten += itemsWritten * sizeof(int);
  ...
  for(int setIndex = 1; setIndex < numSets; setIndex++)
  {
    ...
    itemsWritten = 1;
    if (!countBytesOnly)
      itemsWritten = (unsigned int)fwrite(&length, sizeof(int), 1, binaryOutputStream);
    if (itemsWritten < 1)
      return 1;
    totalBytesWritten += itemsWritten * sizeof(int);
    ...
    unsigned int cardinality = (unsigned int)setElements.size();
    itemsWritten = 1;
    if (!countBytesOnly)
      itemsWritten = (unsigned int)fwrite(&cardinality, sizeof(int), 1, binaryOutputStream);
    if (itemsWritten < 1)
      return 1;
    totalBytesWritten += itemsWritten * sizeof(int);
    ...
    if (!countBytesOnly)
      itemsWritten = (unsigned int)fwrite(elementGroup, sizeof(int), cardinality, binaryOutputStream);
  ...
  itemsWritten = 1;
  if (!countBytesOnly)
    itemsWritten = (unsigned int)fwrite(&numRegions, sizeof(int), 1, binaryOutputStream);
  if (itemsWritten < 1)
    return 1;
  totalBytesWritten += itemsWritten * sizeof(int);
  ...
  for(int regionIndex=0; regionIndex < numRegions; regionIndex++)
  {
    ...
    itemsWritten = 1;
    if (!countBytesOnly)
      itemsWritten = (unsigned int)fwrite(&materialIndex, sizeof(int), 1, binaryOutputStream);
    if (itemsWritten < 1)
      return 1;
    totalBytesWritten += itemsWritten * sizeof(int);
    ...
    itemsWritten = 1;
    if (!countBytesOnly)
      itemsWritten = (unsigned int)fwrite(&setIndex, sizeof(int), 1, binaryOutputStream);
    if (itemsWritten < 1)
      return 1;
    totalBytesWritten += itemsWritten * sizeof(int);
  }
...
VolumetricMesh::VolumetricMesh(const VolumetricMesh & volumetricMesh, int numElements_, int * elements_, map<int,int> * vertexMap_)
{
  ...
  numVertices = (int)vertexSet.size();
  vertices = (Vec3d**) malloc (sizeof(Vec3d*) * numVertices);
  set<int> :: iterator iter;
  int vertexNo = 0;
  ...
  for(int oldSetIndex=0; oldSetIndex < volumetricMesh.getNumSets(); oldSetIndex++)
  {
    ...
    if (newElements.size() > 0)
    {
      ...
      oldToNewSetIndex.insert(make_pair(oldSetIndex, int(newSets.size()) - 1));
    }
  }

  numSets = (int)newSets.size();
  sets = (Set**) malloc (sizeof(Set*) * numSets);
  for(int i=0; i<numSets; i++)
    sets[i] = newSets[i];
