...
static const int kMaxTetNum = 300000;
...
IsotropicHyperelasticFEM::IsotropicHyperelasticFEM(TetMesh * tetMesh_, IsotropicMaterial * isotropicMaterial_, double inversionThreshold_, bool addGravity_, double g_) :
  ...
  //  elementK = (double*) malloc(sizeof(double) * 144 * numElements);
  //  elementForce = (double*) malloc(sizeof(double) * 12 * numElements);
  elementK = (double*) malloc(sizeof(double) * 144 * kMaxTetNum);
  elementForce = (double*) malloc(sizeof(double) * 12 * kMaxTetNum);
  ...
  //  Fs = (Mat3d*) malloc (sizeof(Mat3d) * numElements * 2);
  //  Fhats = (Vec3d*) malloc (sizeof(Vec3d) * numElements * 2);
  //  Vs = (Mat3d*) malloc (sizeof(Mat3d) * numElements * 2);
  //  Us = (Mat3d*) malloc (sizeof(Mat3d) * numElements * 2);
...
void IsotropicHyperelasticFEM::GetStiffnessMatrixTopology(SparseMatrix ** tangentStiffnessMatrix, std::vector<std::pair<int,int> > connections) {
    int numVertices = tetMesh->getNumVertices();
    int numElementVertices = tetMesh->getNumElementVertices(); // This will always be 4.
    int * vertices = (int*) malloc (sizeof(int) * numElementVertices);
    ...
    for(auto it = connections.begin(); it!= connections.end(); ++it)  {
        int i = it->first;
        int j = it->second;
        emptyMatrix->AddEntry(3*i+0,3*j+0,0.0);
        emptyMatrix->AddEntry(3*i+1,3*j+1,0.0);
        emptyMatrix->AddEntry(3*i+2,3*j+2,0.0);

        emptyMatrix->AddEntry(3*j+0,3*i+0,0.0);
        emptyMatrix->AddEntry(3*j+1,3*i+1,0.0);
        emptyMatrix->AddEntry(3*j+2,3*i+2,0.0);
    }
    *tangentStiffnessMatrix = new SparseMatrix(emptyMatrix);
    delete(emptyMatrix);
    free(vertices);
    ...
    */
    ...
    if (computationMode & COMPUTE_INTERNALFORCES) {
      ...
      */
      ...
      {
        double* force = elementForce + el * 12;
        force[0] = forceUpdateA[0];
        force[1] = forceUpdateA[1];
        force[2] = forceUpdateA[2];

        force += 3;
        force[0] = forceUpdateB[0];
        force[1] = forceUpdateB[1];
        force[2] = forceUpdateB[2];

        force += 3;
        force[0] = forceUpdateC[0];
        force[1] = forceUpdateC[1];
        force[2] = forceUpdateC[2];

        force += 3;
        force[0] = forceUpdateD[0];
        force[1] = forceUpdateD[1];
        force[2] = forceUpdateD[2];
      }
        ...
        for (int vtxIndexB = 0; vtxIndexB < 4; vtxIndexB++) {
          ...
          for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++) {
              ...
              int ciderMEGA = tangentStiffnessMatrix->GetInverseIndex(row, 3 * vtxB + j);

              tangentStiffnessMatrix->AddEntry(row, ciderMEGA, *value);
            }
        }
    }
  }
...
#if 1
int IsotropicHyperelasticFEM::ComputePartialForceAndStiffnessMatrixWithSplitedMesh(int* tets,
                                                                                   int tet_num,
                                                                                   int* tetId,
                                                                                   int* vertsOnTet,
                                                                                   double* pos) {
  int exitCode = 0;
  OMP_FOR
  for (int idx = 0; idx < tet_num; ++idx) {
    int el = 0;
    int* actualVert = nullptr;
    int tetIdx = 0;
    if (tets == nullptr) {
      tetIdx = idx;
      el = tetId[idx];
      actualVert = vertsOnTet + idx * 4;
    } else {
      tetIdx = tets[idx];
      el = tetId[tetIdx];
      actualVert = vertsOnTet + tets[idx] * 4;
    }

    Vec3d va(pos[actualVert[0] * 3 + 0], pos[actualVert[0] * 3 + 1], pos[actualVert[0] * 3 + 2]);
    Vec3d vb(pos[actualVert[1] * 3 + 0], pos[actualVert[1] * 3 + 1], pos[actualVert[1] * 3 + 2]);
    Vec3d vc(pos[actualVert[2] * 3 + 0], pos[actualVert[2] * 3 + 1], pos[actualVert[2] * 3 + 2]);
    Vec3d vd(pos[actualVert[3] * 3 + 0], pos[actualVert[3] * 3 + 1], pos[actualVert[3] * 3 + 2]);
    ...
    */
...

    {
      ...
      */
      ...
      Vec3d forceUpdateD = P * areaWeightedVertexNormals[4 * el + 3];
      {
        double* force = elementForce + tetIdx * 12;
        force[0] = forceUpdateA[0];
        force[1] = forceUpdateA[1];
        force[2] = forceUpdateA[2];
...
//        if (0)
      {
        ...
        //  Compute_dPdF(el, dPdF, clamped, &Fhat[0]);
        {
          double sigma[3] = {Fhat[0], Fhat[1], Fhat[2]};
          ...
          memset(dPdF_atFhat, 0, sizeof(double) * 81);
          {
            ...
            for (int i = 0; i < 3 && d[i] < 0; ++i) {
              d[i] = 0;
              corrected = true;
            }
            double tmp[3][3];
            if (corrected) {
              tmp[0][0] = d[0] * v[0][0]; tmp[1][0] = d[0] * v[1][0]; tmp[2][0] = d[0] * v[2][0];
              tmp[0][1] = d[1] * v[0][1]; tmp[1][1] = d[1] * v[1][1]; tmp[2][1] = d[1] * v[2][1];
              tmp[0][2] = d[2] * v[0][2]; tmp[1][2] = d[2] * v[1][2]; tmp[2][2] = d[2] * v[2][2];
              dj::MulMatrixLeftTransposed3x3<double>(v, tmp, A);
              double x1111 = A[0][0];
              double x2222 = A[1][1];
              double x3333 = A[2][2];
              double x2211 = A[0][1];
...

          {
            ...
            double v[2][2];
            double d[2];
            eigen_decomposition<2>((double (*)[2]) &A[0][0], (double (*)[2]) &v[0][0], d);
            bool corrected = false;
            ...
            double tmp[2][2];
            if (corrected) {
              tmp[0][0] = d[0] * v[0][0]; tmp[1][0] = d[0] * v[1][0];
              tmp[0][1] = d[1] * v[0][1]; tmp[1][1] = d[1] * v[1][1];
              A[0][0] = v[0][0] * tmp[0][0] + v[1][0] * tmp[1][0];
              A[0][1] = v[0][0] * tmp[0][1] + v[1][0] * tmp[1][1];
              A[1][0] = v[0][1] * tmp[0][0] + v[1][1] * tmp[1][0];
              A[1][0] = v[0][1] * tmp[0][1] + v[1][1] * tmp[1][1];
          ...
          Mat3d UT = trans(U); // trans(*U);
          Mat3d VT = trans(V); // trans(*V);
        ...
        //                     &(areaWeightedVertexNormals[4 * el + 2]), dPdF, dGdF);
        {
          ...
          Vec3d * bVec[3] = {
            &(areaWeightedVertexNormals[4 * el + 0]),
            &(areaWeightedVertexNormals[4 * el + 1]),
            &(areaWeightedVertexNormals[4 * el + 2]),
          };
          memset(dGdF, 0, sizeof(double) * 81);
...
//  OMP_FOR
  for (int idx = 0; idx < tet_num; ++idx) {
    ...
    Vec3d ds3 = vd - vc;
//    if (0 &&  idx == 0) {
//      P(dj::Vec3d(pos + actualVert[0] * 3));
//      P(dj::Vec3d(pos + actualVert[1] * 3));
//      P(dj::Vec3d(pos + actualVert[2] * 3));
//      P(dj::Vec3d(pos + actualVert[3] * 3));
//      P(dj::Vec3d(&ds1[0]));
//      P(dj::Vec3d(&ds2[0]));
//      P(dj::Vec3d(&ds3[0]));
//    }
...
int IsotropicHyperelasticFEM::ComputePartialForceAndStiffnessMatrix(std::vector<int>& tets, double* u) {
  //printf("Entering IsotropicHyperelasticFEM::GetEnergyAndForceAndTangentStiffnessMatrixHelperWorkhorse\n");
  //printf("inversionThreshold=%G\n", inversionThreshold);
  ...
  for (int i = 0; i < int(tets.size()); ++i) {
    ...
    for (int j = 0; j < 3; ++j) {
      currentVerticesPosition[vaIndex + j] = restVerticesPosition[vaIndex + j] + u[vaIndex + j];
      currentVerticesPosition[vbIndex + j] = restVerticesPosition[vbIndex + j] + u[vbIndex + j];
      currentVerticesPosition[vcIndex + j] = restVerticesPosition[vcIndex + j] + u[vcIndex + j];
      currentVerticesPosition[vdIndex + j] = restVerticesPosition[vdIndex + j] + u[vdIndex + j];
    }
...
void IsotropicHyperelasticFEM::Compute_dPdF(int el, double dPdF[81], int clamped) {
  ...
  */
  ...
  double dPdF_atFhat[81];
  memset(dPdF_atFhat, 0, sizeof(double) * 81);
#define ENSURE_SEMI_POSITIVE_DEFINITE
#ifndef ENSURE_SEMI_POSITIVE_DEFINITE
  dPdF_atFhat[tensor9x9Index(0, 0, 0, 0)] = x1111;
  dPdF_atFhat[tensor9x9Index(0, 0, 1, 1)] = x2211;
  dPdF_atFhat[tensor9x9Index(0, 0, 2, 2)] = x3311;
...
//void IsotropicHyperelasticFEM::Compute_dPdF(int el, double dPdF[], int clamped, double *fhats,
//                                            Mat3d* Us, Mat* Vs) {
//}
