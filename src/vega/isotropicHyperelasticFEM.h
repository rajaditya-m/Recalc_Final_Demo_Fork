...
public:
  ...
  //Overloaded version by the great RAJ
  void GetStiffnessMatrixTopology(SparseMatrix ** tangentStiffnessMatrix, std::vector<std::pair<int,int> > connections);
  ...
  int ComputePartialForceAndStiffnessMatrix(std::vector<int> &tets, double *u);
  int ComputePartialForceAndStiffnessMatrixWithSplitedMesh(int* tets, int tet_num, int* tetId, int* vertsOnTet, double* pos);
...
//  void Compute_dPdF(int el, double dPdF[81], int clamped, double* fhats, Mat3d *Us, Mat *Vs);
