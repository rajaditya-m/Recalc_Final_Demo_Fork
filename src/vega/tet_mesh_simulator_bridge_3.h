...
public:
  typedef double real;
  TetMeshSimulatorBridge(Tet* tet_mesh, std::vector<std::pair<int,int> > &interfaceV);
  ...
  SparseMatrix* GetStitchStiffnessMatrixPointer();
  ...
  std::vector<std::pair<int,int> > interface_points_;
  int sim_mode_;
