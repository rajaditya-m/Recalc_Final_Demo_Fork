 ...
 *                                                                       *
 * "matrixIO" library , Copyright (C) 2007 CMU, 2009 MIT, 2014 USC       *
 * All rights reserved.                                                  *
 *                                                                       *
...
int MultiMatrixIO::Load(const char * filename, int * numMatrices, int ** m, int ** n, double *** matrices)
{
  ...
  unsigned int items = (unsigned int)fread(numMatrices, sizeof(int), 1, fin);
  if (items != 1)
  {
  ...
  *n = (int*) malloc (sizeof(int) * (*numMatrices));
  ...
  for(int i=0; i<*numMatrices; i++)
  {
    items = (unsigned int)fread(&((*m)[i]), sizeof(int), 1, fin);
    if (items != 1)
    {
      printf("Error in MultiMatrixIO::Load: mismatch in the number of matrices.\n");
      return 1;
    ...
    items = (unsigned int)fread(&((*n)[i]), sizeof(int), 1, fin);
    if (items != 1)
    {
      printf("Error in MultiMatrixIO::Load: mismatch in the number of matrices.\n");
      return 1;
  ...
  items = (unsigned int)fread(mem, sizeof(unsigned char), totalBytes, fin);
  if (items != totalBytes)
  {
